...
- **Pedidos programados:** `scheduleSlots` (en `services/schedule.ts`) calcula las horas a las que el negocio puede tener el pedido listo, contra su horario REAL y en hora de Ecuador — nunca antes de `ahora + preparación`, nunca la hora de cierre exacta, y **nada si no hay horario cargado** (ofrecer las 24 horas de un local que abre seis es peor que no ofrecer). El catálogo las manda con el resto para ahorrar un viaje. `isValidSlot` las vuelve a comprobar al crear el pedido: la lista es una comodidad para elegir, no una autorización. **Con el local `cerrada` SÍ se acepta un pedido programado** —a las once de la noche es cuando alguien decide qué va a comer mañana—, pero `suspendida` y `no_disponible` no se saltan ni programando: son cosas distintas.
