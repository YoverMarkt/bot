...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- La migración anterior añadió el método al CHECK de la tabla y a la ruta,
-- pero `create_storefront_order` lleva su PROPIA lista de métodos válidos y
-- seguía rechazándolo con «Metodo de pago invalido». El pedido no llegaba a
-- crearse: la validación de la función se dispara antes que el CHECK.
--
-- Salió al probarlo contra la base real, no al escribirlo. Un valor permitido
-- en dos sitios y prohibido en un tercero no falla al compilar ni en los tests
-- con simulacros: falla cuando un cliente intenta pedir.
--
-- Va en un archivo NUEVO y no editando el anterior porque aquel ya está
-- aplicado, y el registro de migraciones lo impide a propósito: una huella que
-- cambia después de aplicarse deja la base y el historial contando cosas
-- distintas.
--
-- Firma idéntica. Idempotente. Aplicar con `npm run migrate`.
