...
// ── Migraciones que DEPENDEN de otra, declarado en vez de insinuado ────────
//
// El orden base es alfabético, así que hasta ahora la única forma de forzar
// que una migración corriera después de otra era BAUTIZARLA para que cayera
// más tarde en el alfabeto. CLAUDE.md lo dice de las dos que están aquí
// abajo: «⚠️ se llama así para ordenar DESPUÉS de las categorías, de las que
// depende», y de la otra: «es la TERCERA vez que esa trampa aparece y el CI
// no puede verla».
//
// Eso funciona hasta que alguien renombra un archivo por claridad, o añade
// uno que se cuela en medio — y entonces la migración corre antes que aquello
// de lo que depende, y falla con un error que habla de una tabla ausente en
// vez de decir lo que pasó. Declararlo lo vuelve comprobable.
//
// ⚠️ Aquí NO se listan las dependencias implícitas de todas las migraciones:
// solo aquellas cuyo orden correcto hoy depende de una coincidencia del
// alfabeto. Una lista exhaustiva sería imposible de mantener y daría
// confianza falsa.
//
// ⚠️ Las tres «fases de retiro» (hospedaje → citas → modo menú) que vivían
// aquí se RETIRARON el 2026-08-24: las dos primeras están aplicadas desde el
// 2026-08-16, y `migration-2026-08-16-retirar-modo-menu.sql` NUNCA EXISTIÓ
// —la retirada real fue `migration-2026-08-21-retirar-el-modo-ia.sql`, otro
// nombre y otra fecha—. Vigilaban un orden que ya ocurrió y un archivo
// fantasma. Umbani es solo domicilios: hospedaje y citas no existen.
const DEPENDENCIAS_POR_MIGRACION = new Map([
  [
    'migration-2026-08-21-marketplace-busqueda.sql',
    ['migration-2026-08-21-categorias-del-marketplace.sql'],
  ],
  [
    'migration-2026-08-22-lectura-del-comprobante.sql',
    ['migration-2026-08-22-huella-del-comprobante.sql'],
  ],
])
