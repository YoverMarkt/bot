...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Dos cosas que la tienda contaba mal, y una que no contaba.
--
-- ── 1. «Pedido confirmado» era mentira ────────────────────────────────────
--
-- La pantalla lo decía en el instante de crearlo, cuando el estado real era
-- `pendiente` y el negocio no lo había mirado todavía. El cliente creía tener
-- un compromiso que nadie había dado. La app pasa a decir «pedido realizado» y
-- a llevarlo al seguimiento, donde ve el estado de verdad.
--
-- ── 2. Todo pedido nacía igual ────────────────────────────────────────────
--
-- `esperando_pago` existía desde hacía tiempo y NO LO USABA NADIE: daba igual
-- pagar en efectivo en la puerta que deber una transferencia. El dueño veía lo
-- mismo en dos situaciones muy distintas. Ahora quien va a transferir nace
-- esperando el pago.
--
-- ── 3. «Aceptar y preparar» era imposible ─────────────────────────────────
--
-- `pago_en_revision → preparacion` estaba prohibida a propósito («nunca
-- directo a la cocina»), porque aceptar el pago y empezar a cocinar eran dos
-- decisiones. Con un solo botón son UNA: el dueño que da el pago por bueno es
-- el mismo que manda hacerlo, y el paso intermedio solo añadía un estado que
-- el cliente no entiende. Se abre también desde `esperando_pago`, para el caso
-- real de quien transfiere y avisa por WhatsApp sin subir nada.
--
-- Rechazar sigue siendo la otra salida, y ningún estado retrocede.
--
-- Ninguna de las dos funciones cambia de FIRMA. Idempotente.
