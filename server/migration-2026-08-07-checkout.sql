...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Dos huecos de la pantalla 9 del diagrama.
--
-- ── 1. Instrucciones ──────────────────────────────────────────────────────
--
-- «Llame al llegar», «timbre roto, toque la puerta», «dejar en portería». Hoy
-- eso se perdía: la dirección tiene su referencia fija —la del sitio— pero no
-- había dónde poner lo que cambia de un pedido a otro.
--
-- ⚠️ `create_storefront_order` YA recibía `p_notes` y lo tiraba: el parámetro
-- estaba en la firma desde siempre y el insert nunca lo guardaba. Así que aquí
-- no se añade ningún parámetro —la firma no cambia, el `create or replace`
-- reemplaza de verdad— solo la columna que faltaba y el insert que la llena.
--
-- ── 2. Pago al retirar ────────────────────────────────────────────────────
--
-- El tercer método del diagrama. Es distinto de «efectivo»: no es cómo paga,
-- es CUÁNDO — al pasar por el local. La app solo lo ofrece en modo retiro,
-- porque prometer «pago al retirar» a quien pidió a domicilio es ofrecer algo
-- que no puede cumplir.
--
-- ⚠️ El CHECK que manda es este `alter table`, no el del `create table` de más
-- arriba: hay dos y el de abajo pisa al de arriba. Añadir el valor solo en uno
-- lo deja fuera igualmente — es la misma trampa de los estados del pedido.
--
-- La tarjeta sigue sin existir: la plataforma no procesa cobros (regla #6).
--
