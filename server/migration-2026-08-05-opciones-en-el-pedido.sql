-- ============================================================================
-- LAS OPCIONES ENTRAN AL PEDIDO
--
-- El motor de opciones existía en la base desde el 2026-08-04 y NADIE lo leía:
-- ni el servidor, ni la mini app, ni esta función. `order_item_options` —la
-- fotografía de lo que el cliente eligió— llevaba tres migraciones vacía.
--
-- Esta versión de `create_storefront_order` la llena, y hace lo que ninguna
-- capa hacía todavía:
--
--   · RECALCULA el recargo de cada opción contra `options`, nunca contra lo
--     que mande el navegador (regla inviolable #8).
--   · COMPRUEBA que la opción pertenece a un grupo de ESTE producto —suyo o de
--     su categoría—, así que no se puede abaratar una pizza mandando el id de
--     una opción de otro plato.
--   · EXIGE los grupos obligatorios y sus mínimos. Un pedido sin el término de
--     la carne llega a la cocina sin poder prepararse. La app ya lo impide,
--     pero la app se puede saltar.
--
-- ── Qué se conserva del camino viejo ────────────────────────────────────────
-- `extra_ids` sigue funcionando igual: `menu_modifiers` no se retira aquí y el
-- modo menú del bot y el panel del dueño siguen usándola. Un cliente con la app
-- vieja cacheada en el móvil durante el despliegue sigue pudiendo pedir.
--
-- Y `order_items.extras_names` se sigue llenando —ahora también con las
-- opciones—, porque es lo que el DUEÑO ve en su panel de pedidos. Si las
-- opciones solo fueran a la tabla nueva, el pedido se vería sin lo que el
-- cliente pidió. Es el mismo descuido que se llevó el check azul el 2026-08-03.
--
