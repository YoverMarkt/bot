...
- **Los grupos del pedido, en el orden que configuró el dueño.** Hoy `services/order-detail.ts` los ordena **alfabéticamente**, y funciona: es determinista y el mismo plato se lee igual siempre. Pero el orden bueno sería `option_groups.sort` — el que puso el dueño en su panel y el que vio el cliente al armar el plato, donde «Sabor» va antes que «Borde» porque así se piensa una pizza. No se puede resolver al leer: `order_item_options` guarda `option_group_id` pero **sin foránea**, así que PostgREST no puede traer el `sort` incrustado, y hacerlo con una consulta aparte añade un viaje en las tres superficies. La forma limpia es **copiar el `sort` al insertar**, como ya se copian el nombre del grupo y el precio — y eso obliga a **recrear `create_storefront_order`**, la función que cobra. No compensa arriesgar la RPC del dinero por el orden de cinco líneas; se hace cuando haya otro motivo para tocarla y se aprovecha el viaje. Va con **base-de-datos** + **arquitecto-saas**.
